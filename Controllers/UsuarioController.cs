    ...
    public class UsuarioController : ApiController
    {
        ...
        public IEnumerable<dynamic> Get()
        {
            using (pro_chef_webEntities bd = new pro_chef_webEntities())
            {
                var usuario = from user in bd.dados_cliente
                              select new { user.id, user.cpf, user.nome, user.telefone, user.usuario, user.senha, user.cep, user.logradouro, user.complemento, user.bairro, user.cidade, user.uf, user.tipo_usuario, user.numero };
        ...
        public string Put(int id, [FromBody]dados_cliente user)
        {
            using(pro_chef_webEntities bd = new pro_chef_webEntities())
            {
                ...
                userAlterar.numero = user.numero;
